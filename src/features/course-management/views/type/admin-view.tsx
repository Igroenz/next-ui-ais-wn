...
const AdminCourseTypeView = ({ role }: { role: string }) => {

  const [openDialogForm, setOpenDialogForm] = useState<boolean>(false);
  const [openDialogDelete, setOpenDialogDelete] = useState<boolean>(false);
  const [selectedData, setSelectedData] = useState<Partial<CourseType> | null>(null);
  const [isEditMode, setIsEditMode] = useState<boolean>(false);

  const { courseTypes, createCourseType, updateCourseType, deleteCourseType } = useCoursesType()
  ...
  const handleEdit = (data: Partial<CourseType>) => {
  ...
  const handleDelete = (id: string | number) => {
    const data = courseTypes.find(p => p.id === id);
    if (data) {
      setSelectedData(data);
  ...
  const handleConfirmDelete = () => {
    if (selectedData) {
      try {
        deleteCourseType("ADMIN", selectedData.id!);
  ...
  const handleSubmit = (data: Omit<CourseType, "id" | "created_at" | "updated_at" | "deleted_at">) => {
    try {
      if (selectedData && isEditMode) {
        updateCourseType("ADMIN", selectedData.id!, data);
      } else {
        createCourseType("ADMIN", data);
      }
  ...
  return (
    <div className="bg-white p-4 lg:p-6 rounded-md flex-1 m-4 lg:mt-16 space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto justify-between">
        <h1 className="block text-xl font-semibold self-start">Daftar Tipe Mata Kuliah</h1>
...
export default AdminCourseTypeView;